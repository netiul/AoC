<?php

namespace Netiul\AoC\Y2017\Day1\Part2;

$captchas = [
    '1212',
    '1221',
    '123425',
    '123123',
    '12131415',
    '',
];

foreach ($captchas as $captcha) {
    echo Captcha::solve($captcha);
}

class Captcha
{
    const REGEX = '(\d{1})(?=\d{%s}\1)';

    /**
     * @param string $captcha
     * @return CaptchaResult
     */
    public static function solve(string $captcha): CaptchaResult
    {
        $halfCaptchaLength = strlen($captcha) / 2;
        $pattern = sprintf(self::REGEX, $halfCaptchaLength - 1);
        preg_match_all("/$pattern/", $captcha . substr($captcha, 0, $halfCaptchaLength), $matches, PREG_OFFSET_CAPTURE);

        $digitPairs = array_column($matches[1], 0);
        $formattedCaptcha = self::format($captcha, array_reverse(array_column($matches[1], 1)));
        $sum = self::calculateSum($digitPairs);

        return new CaptchaResult($captcha, $formattedCaptcha, $digitPairs, $sum);
    }

    /**
     * @param array $digitPairs
     * @return int
     */
    private static function calculateSum(array $digitPairs): int
    {
        return array_sum($digitPairs);
    }

    /**
     * @param string $captcha
     * @param array $reverseOffset
     * @return string
     */
    private static function format(string $captcha, array $reverseOffset): string
    {
        array_walk($reverseOffset, function ($offset) use (&$captcha) {
            $captcha = sprintf("%s\x1B[31m" . $captcha[$offset] . "\e[0m%s", substr($captcha, 0, $offset),  substr($captcha, $offset + 1));
        });

        return $captcha;
    }
}

class CaptchaResult
{
    /**
     * @var string
     */
    private $captcha;

    /**
     * @var string
     */
    private $formattedCaptcha;

    /**
     * @var int
     */
    private $sum;

    /**
     * @var array
     */
    private $digitParis;

    /**
     * @param string $captcha
     * @param string $formattedCaptcha
     * @param array $digitPairs
     * @param int $sum
     */
    public function __construct(string $captcha, string $formattedCaptcha, array $digitPairs, int $sum)
    {
        $this->captcha = $captcha;
        $this->formattedCaptcha = $formattedCaptcha;
        $this->sum = $sum;
        $this->digitParis = $digitPairs;
    }

    /**
     * @return int
     */
    public function getSum(): int
    {
        return $this->sum;
    }

    public function __toString(): string
    {
        $captcha = $this->captcha;
        $formattedCaptcha = $this->formattedCaptcha;
        $digitPairs = $this->digitParis;

        return "\n" . str_repeat("=", 80) . "\n" .
            "Captcha:                $captcha\n" .
            "Formatted captcha:      $formattedCaptcha\n" .
            "Digit pairs:            \x1B[31m" . implode('', $digitPairs) . "\n\e[0m" .
            "Sum of digits:          " . array_sum($digitPairs) . "\n";
    }
}
